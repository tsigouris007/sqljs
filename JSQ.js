function stripTags(sql) {
    return sql.replace(/(<([^>]+)>)/gi, "");
}

function sanitize(sql) {
    sql = stripTags(sql).replaceAll("\r", "").replaceAll("\n", " ").replaceAll("\t", "").trim()
    return sql;
}
...
JSQ.generator= {};
    ...
    JSQ.parser.parse = function(sql_in) {
        ...
        {
            word = sql[i];
            if (word == "#") break; // Comments do not get parsed
            ...
            if(tokens.select.exists && tokens.from.exists && !tokens.where.exists)
            {
                if(JSQ.parser.isNotSpecialWord(word))
                {
                    if(word.toString().endsWith(",") || tokens.from.full)
